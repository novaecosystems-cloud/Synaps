...
export const AcidSquares: React.FC<AcidSquaresProps> = ({
  color1 = '#0284c7', // Causarix signature cyan/azure
  color2 = '#fc4778', // Causarix signature neon coral/pink
  color3 = '#ffffff', // Crisp specular highlight
  detail = 'medium',
  speed = 0.7,
  waveDepth = 1,
  zoom = 1.3,
  ...
  useEffect(() => {
    ...
    try {
      ...
      console.warn('[AcidSquares] WebGL 2 not supported or failed to initialize:', e);
      return;
    }
    ...
    let rtB: any = null;
    const ensureTargets = () => {
      if (!rtA && gl.drawingBufferWidth > 0 && gl.drawingBufferHeight > 0) {
    ...
    return () => {
      ...
      if (rtA) {
        try {
          ...
          rtA.textures?.forEach((tex: any) => gl.deleteTexture(tex.texture));
          rtB.textures?.forEach((tex: any) => gl.deleteTexture(tex.texture));
        } catch {}
      }
      try {
