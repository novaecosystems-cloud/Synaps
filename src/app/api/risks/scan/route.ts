...
export async function POST(req: NextRequest) {
  try {
    ...
    // Enforce 2-Use IP Trial Quota for Risk Scan and Redline
    const { checkAndConsumeAiCredits, extractClientIp } = await import('@/lib/ai-credit-limiter');
    const clientIp = extractClientIp(req.headers);
    const creditCheck = await checkAndConsumeAiCredits(decoded.uid, dbUser?.role || 'MEMBER', 1, 'contract_redline', clientIp);
