...
pdf_path = r"C:\Users\Shourya\Pictures\pitch deck\Synaps_Pitch_Deck_v2.pdf"
...
class DarkCanvas(canvas.Canvas):
    ...
    def draw_footer(self, total_pages):
        ...
        self.drawString(36, 18, "SYNAPS AI · ASYNCHRONOUS INVESTOR PITCH DECK")
        self.drawRightString(792 - 36, 18, f"SLIDE {self._pageNumber} / {total_pages}")
        self.setStrokeColor(colors.HexColor('#1e293b'))
        self.setLineWidth(0.5)
        self.line(36, 30, 792 - 36, 30)
        self.restoreState()

# Page Setup (11in x 8.5in landscape = 792 x 612 pt)
...
# Compact Card Helper with tight padding so NO wasted space exists
def make_card(header_text, body_text, border_color="#38bdf8", width=225):
    h = Paragraph(f"<b>{header_text}</b>", ParagraphStyle('CardH', fontName='Helvetica-Bold', fontSize=11, leading=15, textColor=colors.HexColor(border_color)))
...
btn_table = Table([
    [
        Paragraph("<font color='#ffffff'><b>ZERO HALLUCINATIONS</b></font>", ParagraphStyle('Btn1', fontName='Times-Bold', fontSize=11, alignment=1)),
        Paragraph("<font color='#38bdf8'><b>LINE-LEVEL CITATIONS</b></font>", ParagraphStyle('Btn2', fontName='Times-Bold', fontSize=11, alignment=1))
    ]
], colWidths=[190, 190])
...
c2 = make_card("2. NO LINE-LEVEL AUDIT TRAIL", "Generic ChatGPT prompts give text answers without verifiable proof. Leaders cannot verify underlying contract sources or audit hashes.", "#f87171", 225)
...
s3 = make_card("🧮 PRIME RLM ENGINE", "Process-Outcome step-by-step verification achieving 99.4% math proof accuracy on PutnamBench &amp; AIME benchmarks.", "#fbbf24", 225)
...
img0 = Image(screenshots[0], width=380, height=250) if len(screenshots) > 0 else Paragraph("Dashboard Image", body_style)
...
feat1_table = Table([[img0, desc_card]], colWidths=[390, 325])
...
img2 = Image(screenshots[2], width=380, height=250) if len(screenshots) > 2 else Paragraph("Legal SLA Image", body_style)
...
sla_card = make_card(
    ...
    "• <b>Single-Click Master PDF Download:</b> Instant client-side printable legal packet.",
    "#34d399",
    310
)

feat2_table = Table([[sla_card, img2]], colWidths=[325, 390])
...
img4 = Image(screenshots[4], width=380, height=250) if len(screenshots) > 4 else Paragraph("Studio Image", body_style)
...
feat3_table = Table([[img4, studio_card]], colWidths=[390, 325])
...
matrix_data = [
    [
        Paragraph("<b>Capability / Feature</b>", ParagraphStyle('TH1', fontName='Times-Bold', fontSize=10, textColor=colors.HexColor('#38bdf8'))),
        Paragraph("<b>ChatGPT / Generic LLMs</b>", ParagraphStyle('TH2', fontName='Times-Bold', fontSize=10, textColor=colors.HexColor('#f87171'))),
        Paragraph("<b>Synaps AI Platform</b>", ParagraphStyle('TH3', fontName='Times-Bold', fontSize=10, textColor=colors.HexColor('#34d399')))
    ],
    [
...
print("ReportLab PDF Pitch Deck generated successfully at:", pdf_path)
